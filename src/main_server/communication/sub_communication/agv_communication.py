...
# AGV 통신
class AgvCommunication(TcpSocketFrame):
    def __init__(self, program_name: str, parameter: dict,
        ...
        self.agv_number = parameter.get("agv_number")
    ...
    def recv_data_process(self, recv_data):
        header_len = len(self.header_code)
        protocol_header_len = header_len + 2
        while len(recv_data) >= protocol_header_len:
            try:
                if recv_data[:header_len] == setSpareData(self.header_code.encode('utf-8'), header_len):
                    # 헤더 코드와 맞을 경우
                    data_length: int = getIntData(recv_data[header_len:protocol_header_len])
                    if len(recv_data) >= data_length + protocol_header_len + 1:
                        data: bytes = recv_data[protocol_header_len:data_length+protocol_header_len]
                        checksum: int = recv_data[data_length+protocol_header_len]
                        recv_data = recv_data[data_length+protocol_header_len+1:]
                        clac_checksum = calculate_checksum(data)
                        if clac_checksum == checksum:
                            # 체크섬 비교 후 맞다면
                            try:
                                uncompressed_data = snappy.uncompress(data)
                                data_dict = orjson.loads(uncompressed_data)
                                self.received_data_process(data_dict)
                            except snappy.snappy.UncompressError:
                                raise Exception("데이터 압축 해제 실패")
                        else:
                    ...
                    # 헤더 코드와 맞지 않을 경우 끝내기
                    raise Exception(f'헤더 코드가 맞지 않습니다. 수신 헤더: {self.data_buffer[:header_len]}')
    ...
    def received_data_process(self, data_dict: dict):
        ...
        if send_type != 'AGV' or number != self.agv_number:
